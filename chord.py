...
DELTA_REPLICATION_K      = 1         #number of replicas K/2 into each direction
...
class Node:
    ...
    #Messages are rourted until their location = dest or the first node with id > dest.
        ...
        if REPLICATION_TYPE == 'delta' and msg.type == 'lookup':
            ...
            first = self.nw.ids[(key_index-(DELTA_REPLICATION_K)-1)%(len(self.nw.ids)-1)]
            last  = self.nw.ids[(key_index+(DELTA_REPLICATION_K))%(len(self.nw.ids)-1)]
            if between(msg.dest, first, key_index):
                NUM_REPLICAS +=1
                msg.arrive()
                return
